...
public class Principal extends javax.swing.JFrame {
    ...
    private boolean isValidDocumento(String numeroDocumento) {
        // Verificar si el número de documento no está vacío
        if (numeroDocumento.isEmpty()) {
            return false;
        }

        // Verificar si el número de documento tiene el formato correcto (por ejemplo, 8 dígitos)
    ...
    private boolean validarCampos() {
        ...
        if (!isValidDocumento(numeroDocumento)) {
            JOptionPane.showMessageDialog(this, "Número de documento no válido");
            return false;
        }

       /* if (!isValidFechaEmisionDni(fechaEmisionDni)) {
            JOptionPane.showMessageDialog(this, "Fecha de emisión del DNI no válida");
            return false;
        }
    ...
    /*private boolean isValidFechaEmisionDni(Date fechaEmisionDni) {
        // Verificar si la fecha de emisión del DNI no está vacía
        if (fechaEmisionDni == null) {
            return false;
        }

        // Verificar si la fecha de emisión del DNI es válida (no es una fecha futura o pasada)
        if (fechaEmisionDni.after(new Date()) || fechaEmisionDni.before(new Date(1900, 1, 1))) {
            return false;
        }
    ...
    private boolean isValidFechaNacimiento(Date fechaNacimiento) {
        // Verificar si la fecha de nacimiento no está vacía
        if (fechaNacimiento == null) {
            return false;
        }

        // Verificar si la fecha de nacimiento es válida (no es una fecha futura o pasada)
        if (fechaNacimiento.after(new Date()) || fechaNacimiento.before(new Date(1900, 1, 1))) {
            return false;
        }
